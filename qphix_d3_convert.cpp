...
#pragma omp parallel default(shared) num_threads(nThreads)
	...
	for(int dir=0; dir<4; ++dir) if(!local_dir[dir])
    	...
    	for(int i=tid; i<qphix_even_sites_on_node; i+=nThreads)
    	{
            ...
            int ind = t2*Pxyz+z2*Pxy+y2*Vxh+x2;
            int v = ((t1*nGZ+z1)*nGY+y1)*nGX+x1;
	    int xodd = (i/Nxh + i/(Nxh*Ny) + i/(Nxh*Ny*Nz)) & 1;
	    ...
	    for(int d=0; d<4; ++d) 
	    {
		    ...
		    dest[ind][2*d][c1][c2][ir][v] = rawsrc[72*(backi+qphix_even_sites_on_node)+18*d+c1*6+c2*2+ir];
		}
	    }
	}
...
#pragma omp parallel default(shared) num_threads(nThreads)
#endif
    {
    	...
    	for(int i=tid; i<qphix_even_sites_on_node; i+=nThreads)
    	{
            ...
            int ind = t2*Pxyz+z2*Pxy+y2*Vxh+x2;
            int v = ((t1*nGZ+z1)*nGY+y1)*nGX+x1;
	    int xodd = (i/Nxh + i/(Nxh*Ny) + i/(Nxh*Ny*Nz) + 1) & 1;
            ...
            for(int d=0; d<4; ++d)
	    {
            	...
            	for(int c2=0; c2<3; ++c2) for(int ir=0; ir<2; ++ir)
            	{
                    dest[ind][2*d+1][c1][c2][ir][v] = rawsrc[72*(i+qphix_even_sites_on_node)+18*d+c1*6+c2*2+ir];
...
#pragma omp parallel default(shared) num_threads(nThreads)
#endif
    {
        int tid = omp_get_thread_num();
        for(int dir=0; dir<4; ++dir) if(!local_dir[dir])
            pack_gforce_face_dir( tid, rawsrc, dir, 0 );
		...
		for(int k=0; k<8; ;++k)
		{
		    ...
		    int ir = k & 1;
		    if(k>=6) ir = 1;
		    dest[ind][2*d+1][k][v] = rawsrc[72*i+18*d+c1*6+c2*2+ir];
                    dest[ind][2*d][k][v] = rawsrc[72*(backi+qphix_even_sites_on_node)+18*d+c1*6+c2*2+ir];
		}
            }
        }

        for(int dir=0; dir<4; ++dir) if(!local_dir[dir])
            unpack_gforce_face_dir( tid, dest, dir, 0 );
    }
    }
    else 
    {
...
#pragma omp parallel default(shared) num_threads(nThreads)
#endif
    {
        int tid = omp_get_thread_num();
        for(int dir=0; dir<4; ++dir) if(!local_dir[dir])
            pack_gforce_face_dir( tid, rawsrc, dir, 1 );
        ...
        for(int i=tid; i<qphix_even_sites_on_node; i+=nThreads)
        {
            ...
            for(int d=0; d<4; ++d)
            {
                ...
                for(int k=0; k<8; ;++k)
                {
                    ...
                    int ir = k & 1;
                    if(k>=6) ir = 1;
                    dest[ind][2*d+1][k][v] = rawsrc[72*(i+qphix_even_sites_on_node)+18*d+c1*6+c2*2+ir];
                    dest[ind][2*d][k][v] = rawsrc[72*backi+18*d+c1*6+c2*2+ir];
                }
	    }
	}

        for(int dir=0; dir<4; ++dir) if(!local_dir[dir])
            unpack_gforce_face_dir( tid, dest, dir, 1 );
    }
    }
#if TIME_GF
    timer +=  hrtimer_get();
...
void QPHIX_D3_extract_G_to_raw( QPHIX_D_Real *dest, QPHIX_D3_GaugeField  *src, QPHIX_evenodd_t eveodd )
{
    /* check parity */
    MYASSERT(src->parity&eveodd);
#if TIME_GF
    timer -= hrtimer_get();
#endif
    Gauge *gf = src->geo;
...
void QPHIX_D3_extract_F_to_raw( QPHIX_D_Real *dest, QPHIX_D3_Force  *src, QPHIX_evenodd_t eveodd )
{
    /* check parity */
    MYASSERT(src->parity&eveodd);
#if TIME_GF
...
#pragma omp parallel default(shared) num_threads(nThreads)
#endif
    {
        int tid = omp_get_thread_num();
        for(int dir=0; dir<4; ++dir) if(!local_dir[dir])
                pack_gforce_face_dir(tid, gf, 2*dir, 0);
		...
		for(int k=0; k<8; ++k)
		{
                    ...
                    int ir = k & 1;
                    if(k>=6) ir = 1;
		    dest[72*i+18*d+6*c1+2*c2+ir] = gf[ind][2*d+1][k][v];
		    dest[72*(backi+qphix_even_sites_on_node)+18*d+6*c1+2*c2+ir] = gf[ind][2*d][k][v];
		}
	    }
	}

	for(int dir=0; dir<4; ++dir) if(!local_dir[dir])
		unpack_gforce_face_dir(tid, dest, 2*dir+1, 0);
    }
    }
    if(evenodd & QPHIX_ODD)
    {
...
#pragma omp parallel default(shared) num_threads(nThreads)
#endif
    {
        int tid = omp_get_thread_num();
        for(int dir=0; dir<4; ++dir) if(!local_dir[dir])
                pack_gforce_face_dir(tid, gf, 2*dir, 1);
        ...
        for(int i=tid; i<qphix_even_sites_on_node; i+=nThreads)
        {
            ...
            int ind = t2*Pxyz+z2*Pxy+y2*Vxh+x2;
            int v = ((t1*nGZ+z1)*nGY+y1)*nGX+x1;
            int xodd = (1 + i/Nxh + i/(Nxh*Ny) + i/(Nxh*Ny*Nz)) & 1;
		...
		for(int k=0; k<8; ++k)
		{
                    ...
                    int ir = k & 1;
                    if(k>=6) ir = 1;
                    dest[72*(i+qphix_even_sites_on_node)+18*d+6*c1+2*c2+ir] = gf[ind][2*d+1][k][v];
                    dest[72*backi+18*d+6*c1+2*c2+ir] = gf[ind][2*d][k][v];
		}
	    }
	}

	for(int dir=0; dir<4; ++dir) if(!local_dir[dir])
		unpack_gforce_face_dir(tid, gf, 2*dir+1, 1);
    }
    }
