...
#define SIZEOF_STRUCT_IN_BUFF  (3*6/2)
#define SIZEOF_STRUCT_IN_BUFF  (3*7*18/2)
/*
...
static int Ls[4] = {0};
static int maxBufSize[4];
static int bufSize[4];
static int gf_bufSize[4];
...
void setup_comms(int myCoord_[4], int neigh_ranks_[8])
{
	...
	for(int i = 0; i < 4; i++) {
		maxBufSize[i] = 1;
		for(int j = 0; j < 4; j++) if(j != i) maxBufSize[i] *= lDims[j];
		maxBufSize[i] *= MAXSIZEOF_STRUCT_IN_BUFF;
		bufSize[i] = maxBufSize[i]*KSDSLASH_STRUCT_IN_BUFF/MAXSIZEOF_STRUCT_IN_BUFF;
		gf_bufSize[i] = maxBufSize[i]*GF_STRUCT_IN_BUFF/MAXSIZEOF_STRUCT_IN_BUFF;
	}
	...
	for(int i = 0; i < 4; i++) {
		alignedBufSize[i] = (maxBufSize[i] + (pgSize-1)) & ~(pgSize-1);
		if(!local_dir[i]) {
	...
	for(int i = 0; i < 4; i++) {
		if(!local_dir[i]) {
			MYASSERT(MPI_Irecv((void *)commsBuf[4*i], totalBufSize*sizeof(fptype), MPI_BYTE, neigh_ranks[2*i+0], DSLASH_MPI_RECV_TAG(0), MPI_COMM_THISJOB, &reqRecvs[2*i+0]) == MPI_SUCCESS);
			MYASSERT(MPI_Isend((void *)commsBuf[4*i], totalBufSize*sizeof(fptype), MPI_BYTE, neigh_ranks[2*i+1], DSLASH_MPI_SEND_TAG(1), MPI_COMM_THISJOB, &reqSends[2*i+1]) == MPI_SUCCESS);
		...
		if(!local_dir[d]) {
			for(int fb = 0; fb < 2; fb++) {
				...
				for(int i = 0; i < 11; i++) {
					...
					MYASSERT(MPI_Isend((void *)commsBuf[2+4*d+1-fb], bufSize[d]*sizeof(fptype), MPI_BYTE, neigh_ranks[2*d+1-fb], DSLASH_MPI_SEND_TAG(1-fb), MPI_COMM_THISJOB, &reqSends[2*d+1-fb]) == MPI_SUCCESS);
					t0 = __rdtsc();
...
/* Gauge Force */
/***************/
void pack_face_1( int tid, int num_cores, int threads_per_core, Gauge *gi, fptype *res, int cb, int dir, int fb )
{
        int nyg_pack = 1;
        if(dir == 0) nyg_pack = 2;
        int lens1[4] = {Vxh, Vy/nyg_pack, Vz, Vt};
	...
	// FIXME!!
        MYASSERT(18/*??*/*pktsize*npkts == gf_bufSize[dir]);
        int pkts_per_core = (npkts + num_cores - 1) / num_cores;
        // Assume thread indexing runs SMT-ID fastest
        // smtid + threads_per_core*cid
        int cid = tid/threads_per_core;
...
*/
		...
		gf_face_pack_dir(giBase, lBuf, rBuf, dir*2+fb);
		//gf_face_pack_dir(giBase+shift, lBuf+6*pktsize, rBuf+6*pktsize, dir*2+fb);
...
void pack_face_2( int tid, int num_cores, int threads_per_core, Gauge18 *gi, fptype *res, int cb, int dir, int fb )
{
...
/* Gauge force */
/***************/
void pack_and_send_boundaries( )
{

}

void recv_and_unpack_boundaries( )
{
