...
void gf_pack_and_send_boundaries_u(int tid, Gauge *gi, int cb){gBar->wait(tid);}
void gf_recv_and_unpack_boundaries_h(int tid, Hermit *hio, HermitHelperYZT *htmp, Gauge *gio, int cb){gBar->wait(tid);}
void gf_recv_and_unpack_u_and_send_boundaries_h(int tid, Hermit *hio, HermitHelperYZT *htmp, Gauge *gio, fptype kappaS, fptype kappaR, fptype kappaB, fptype epsilonH, int cb){gBar->wait(tid);}
#if QPHIX_PrecisionInt == 1
...
static void gf_unpack_face_u_dir( int tid, int num_cores, int threads_per_core, fptype *gi, fptype *res, int cb, int dir, int fb )
{
	...
	int pkts_per_core = (npkts + num_cores - 1) / num_cores;
        ...
        for(int pkt = low_pkt + smtid; pkt < high_pkt; pkt+=threads_per_core) {
		    ...
		    int ind0 = (((coord0[3]*Nz+coord0[2])*Ny+coord0[1])*Nxh+coord0[0])*72;
		    for(int i=0; i<12; ++i)
			res[ind0+18*dir+i] = gi[(12*pkt+i)*pktsize+vi];
		    reconstruct_gauge_third_row(&res[ind0+18*dir], 0, 0, 0);
		    vi++;
		}
	}
}
