...
QPHIX_F3_create_M(QPHIX_evenodd_t parity)
{
  int p;
  int pmap[2];
...
//  for(p=0; p<2; p++)
  for(p=0; p<1; p++)
  {
    if (pmap[0])
    {
      m1->even = (QSU3M_float *) _mm_malloc(qphix_even_sites_on_node * sizeof(QSU3M_float), VECLENBYTES);
      MYASSERT(m1->even != NULL);
    }
    if (pmap[1])
    {
      m1->odd  = (QSU3M_float *) _mm_malloc(qphix_even_sites_on_node * sizeof(QSU3M_float), VECLENBYTES);
...
QPHIX_F3_create_V(QPHIX_evenodd_t parity)
{
    int p;
    int pmap[2];
    ...
    QPHIX_F3_ColorVector *v1 = (QPHIX_ColorVector *) malloc(sizeof(QPHIX_ColorVector));
    MYASSERT(v1 != NULL);
...
#if TIME_CG
    timer +=  hrtimer_get();
    //if(myRank==0) printf("QPHIX_F3_create_V_from_raw: time = %.6e sec\n ", (timer)/1.e+9);
    //fflush(stdout);
#endif
    return V;
}
...
QPHIX_F3_GaugeField  
*QPHIX_F3_create_G_from_raw( QPHIX_F_Real *rawsrc, QPHIX_evenodd_t evenodd )
{
...
QPHIX_F3_Force  
*QPHIX_F3_create_F_from_raw( QPHIX_F_Real *rawsrc, QPHIX_evenodd_t evenodd )
{
    MYASSERT(evenodd==QPHIX_EVENODD||evenodd==QPHIX_EVEN||evenodd==QPHIX_ODD);
#if TIME_GF
    timer = -hrtimer_get();
...
#if TIME_CG
    timer += hrtimer_get();
    //if(myRank==0) printf("QPHIX_F3_extract_V_to_raw: time = %.6e sec\n ", (timer)/1.e+9);
    //fflush(stdout);
#endif
}
...
void QPHIX_F3_extract_F_to_raw( QPHIX_F_Real *dest, QPHIX_F3_Force  *src, QPHIX_evenodd_t evenodd )
{
    ...
    QPHIX_F_Real *destp = dest + 72*qphix_even_sites_on_node;
...
void QPHIX_F3_layout_from_su3m(QPHIX_F3_ColorMatrix *dest, SU3_F_Matrix *src)
{
  ...
  QSU3M_float *dest_e = (QSU3M_float *)dest->even;
  QSU3M_float *dest_o = (QSU3M_float *)dest->odd;
...
void QPHIX_F3_layout_from_4su3m(QPHIX_F3_ColorMatrix *dest[], SU3_F_Matrix *src)
{
  ...
  QSU3M_float *dest_e[4];
  QSU3M_float *dest_o[4];
  
  for(int dir = 0; dir < 4; dir++){
    dest_e[dir] = (QSU3M_float *)dest[dir]->even;
    dest_o[dir] = (QSU3M_float *)dest[dir]->odd;
  }
...
void QPHIX_F3_layout_to_su3m(SU3_F_Matrix *dest, QPHIX_F3_ColorMatrix *src)
{
  ...
  QSU3M_float *qme = (QSU3M_float *)src->even;
  QSU3M_float *qmo = (QSU3M_float *)src->odd;
...
void QPHIX_F3_layout_to_4su3m(SU3_F_Matrix *dest, QPHIX_F3_ColorMatrix *src[])
{
  ...
  for(int dir = 0; dir < 4; dir++){
    qme[dir] = (QSU3M_float *)src[dir]->even;
    qmo[dir] = (QSU3M_float *)src[dir]->odd;
  }
...
QPHIX_F3_hisq_create_L_from_4su3m(void *U_links, void *V_links, void *W_unitlinks, void *Y_unitlinks,
				  QPHIX_evenodd_t parity)
{
  QPHIX_F3_FermionLinksHisq *qphix_links = 
    (QPHIX_F3_FermionLinksHisq *)malloc(sizeof(QPHIX_F3_FermionLinksHisq));
  MYASSERT( qphix_links != NULL );

  for(int i=0; i<4; i++) {
    qphix_links->U_links[i] = QPHIX_F3_create_M(parity);
    qphix_links->V_links[i] = QPHIX_F3_create_M(parity);
    qphix_links->W_unitlinks[i] = QPHIX_F3_create_M(parity);
    qphix_links->Y_unitlinks[i] = QPHIX_F3_create_M(parity);
  }

  QPHIX_F3_layout_from_4su3m(qphix_links->U_links, (SU3_F_Matrix*)U_links);
  QPHIX_F3_layout_from_4su3m(qphix_links->V_links, (SU3_F_Matrix*)V_links);
  QPHIX_F3_layout_from_4su3m(qphix_links->W_unitlinks, (SU3_F_Matrix*)W_unitlinks);
  QPHIX_F3_layout_from_4su3m(qphix_links->Y_unitlinks, (SU3_F_Matrix*)Y_unitlinks);

  return qphix_links;
}

static void
destroy_4M(QPHIX_F3_ColorMatrix **M){
  if(M == NULL)return;
  for(int dir = 0; dir < 4; dir++)
    if(M[dir] != NULL)QPHIX_F3_destroy_M( M[dir] );
}

void 
QPHIX_F3_hisq_destroy_L( QPHIX_F3_FermionLinksHisq *L )
{
    if(L==0x00) return;
...
QPHIX_F3_create_F_from_anti_hermitmat( void *ptr, int SZ )
{
  ...
  QPHIX_F3_ColorMatrix **dest = 
    (QPHIX_F3_ColorMatrix **)malloc(sizeof(QPHIX_F3_ColorMatrix *)*4);
  MYASSERT( dest != NULL );

  for(int dir = 0; dir < 4; dir++){
    dest[dir] = QPHIX_F3_create_M(QPHIX_EVENODD);
...
void QPHIX_F3_layout_to_anti_hermitmat(void *ptr, QPHIX_F3_ColorMatrix *src[], int SZ)
{
  ...
  for(int dir = 0; dir < 4; dir++){
    src_e[dir] = (QSU3M_float *)src[dir]->even;
    src_o[dir] = (QSU3M_float *)src[dir]->odd;
  }
